...
As seen in the figure above, we used REST in conjuction with a YAML document, to create a REST service that runs on ones local machine. The service can be connected to through any internet broswer. Within the YAML document, there are predefined paths a user can enter for different purposes. For our project the path can be altered by changing /results/<position> to view fantasy football point predictions based on a given postion. When a postion is entered, the HTTP request is then sent to the REST server. Based on the path entered, the REST service the completes the predfined task defined in the Yaml file. When a postion is entered, our service calls a python function. Within that function, the datasets are first retrieved from a Google drive. Next, the three nearest neighbors of each rookie player are found and their previous fantasy football points are averaged. Then the rookies and their predicted points are turned into a data frame that ranks them numerically. Finally, the new data frame is coverted to html and returned where it is printed to the users screen as defined in the yaml file.
